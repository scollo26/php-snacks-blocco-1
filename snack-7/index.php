...
<body>
    <?php 
    // array vuoto che conterrà gli indici degli elementi attivi
    $rndIndex = [];

    foreach ($ads as $key => $value) {
        // ciclo per inserire nell'array gli indici attivi
        if ($value['is_active'] === true) {
            $rndIndex[] = $key;
        };
    };

    // variabile che randomicamente prenderà un elemento dall'array con gli indici
    $randIdx = $rndIndex[rand(0, count($rndIndex) - 1)];
    ?>
    <!-- stampa immagine relativa all'indice sorteggiato -->
    <img src="<?php echo $ads[$randIdx]['image_path'] ?>" alt="">
</body>
</html>